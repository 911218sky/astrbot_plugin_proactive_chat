...
- 移除「真人互動節奏」的正常回覆等待與入站訊息防抖，避免插件改變一般對話回覆流程。
- 保留即時跟進的防抖等待，並將互動熱度設定維持在即時跟進配置中。
...
- 群聊全域配置與群聊個性化會話現在補齊私聊的自動查看、排程、即時跟進、語境感知、分段回覆、TTS、真人互動與習慣時段設定欄位。
- 群聊習慣時段支援自動學習、手動規則開關、自適應時間窗與互動樣本記錄；群聊沉默觸發仍保留作為群聊專屬保護。
...
- 群聊選擇「自適應檢查」時，現在會與私聊使用相同的穩定排程節奏；只有明確選擇「進階：加權隨機」才會使用舊的隨機排程。
- 群聊與私聊的排程方式選項維持一致；自適應模式的互動風格、回訪範圍與提示仍在「自動查看／回訪設定」中調整，分時段規則只在加權隨機模式顯示。
...
- 真人互動節奏新增正常回覆真人等待與使用者訊息安靜等待；連續分段訊息只由最後一段觸發正常 LLM 回覆，舊事件不再進入 LLM。

### 修復 (Fix)

- 明確分離正常回覆的入站等待與 Bot 回覆後的即時跟進等待，避免誤以為 `delay_seconds` 會延遲正常 AI 回覆。

## v2.24.1

### 修復 (Fix)

- 私聊排程選擇「進階：加權隨機」時，WebUI 現在會顯示分時段規則、間隔權重與未回覆衰減設定。
...
- 私聊新增可選真人互動節奏：內容與夜間延遲、互動熱度、未回覆冷卻及每小時／每日上限。
- 人性化狀態持久化，使用者回覆會提升互動熱度並清除冷卻；群聊行為保持不變。

### 改進 (Improve)

- 私聊自動查看啟用時改用穩定的自適應檢查節奏，未回覆會逐步放慢，舊加權隨機模式保留作相容選項。
- 私聊習慣時段改為建立檢查機會，由同一個語境分析 LLM 判斷是否發送，並停用重疊的一般私聊排程。
- 私聊手動習慣規則預設生效，配置中的會話在插件啟動時也會建立習慣任務。
...
- 私聊新增可選的自動查看／回訪模式，沿用既有全域排程並由語境分析 LLM 判斷是否發送。
- 提供熱戀中的情侶、一般、積極、非常積極、不積極、非常不積極六種預設，以及可調整的最短／最長回訪間隔。
- 回訪不發送時只安排下一次檢查，不增加未回覆次數；發送時沿用原有發送、歷史寫回與即時跟進流程。
...
- 使用者新活動、會話停用、免打擾時段、發送未完整成功或控制器輸出無效時，立即停止剩餘跟進，避免過期訊息繼續送出。
- 完成、失敗或取消的會話會釋放發送協調器；舊 revision 不會清除較新的 alias/revision 狀態。
- 即時跟進會依序完成初始發送、狀態收尾、跟進發送與歷史寫回，並只保存實際成功送出的內容。
...
- 修復啟用分段回覆後仍受字數門檻阻擋，導致較短主動訊息不分段的問題。
- 移除分段回覆設定中容易誤解的字數門檻欄位，開啟分段後會直接依分段規則切分。
...
- 真人互動節奏預設等待調整為 1～3 秒；移除未回覆冷卻與每小時／每日主動訊息上限，避免與排程策略重複限制。
- 真人回覆等待支援 0.1 秒精度，讓隨機延遲可產生例如 0.5～0.7 秒的等待。
