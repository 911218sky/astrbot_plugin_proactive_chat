...
- 群聊全域與個性化配置補齊私聊的自動查看、即時跟進、語境感知、分段回覆、TTS 與習慣時段自動學習設定；群聊仍保留沉默觸發時間作為額外保護
...
互動熱度設定位於各會話的 `immediate_follow_up_settings`：`initial_heat_score` 預設 50，`user_activity_delta` 預設 15，`proactive_delivery_delta` 預設 -5，分數固定限制在 0 到 100。啟用即時跟進後，LLM 會看到目前熱度；正常回覆依原本流程處理。
