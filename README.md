...
- 新增 AstrBot Pages 任務儀表板，可直接查看目前排程、語境任務與自動觸發任務
...
- **任務儀表板**：可在 AstrBot WebUI 查看目前仍在等待執行的任務
...
`decay_rate` 留空時代表不衰減，也就是每次排程到點都允許觸發。填 `"0"` 時代表第一次主動訊息後不再觸發發送，但任務仍會依排程檢查並重排。
...
儀表板提供任務數量摘要、搜尋、類型篩選、手動刷新與自動刷新，方便確認 Bot 目前還會在什麼時間主動發訊息。
