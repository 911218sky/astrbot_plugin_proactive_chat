...
- 新增可選的 AI 即時跟進：每次主動訊息後由 AI 判斷是否再傳下一句，單輪最多追加 0 到 3 句
...
| `immediate_follow_up_settings` | 由 AI 判斷主動訊息後是否立即追加下一句 |
| `history_settings` | 控制是否把主動訊息寫回 AstrBot 主對話歷史 |
| `segmented_reply_settings` | 長訊息切段發送 |
| `tts_settings` | 啟用語音輸出 |

`immediate_follow_up_settings` 預設關閉。開啟後，AI 會在每次已成功發送的主動訊息後判斷「是否還需要下一句」；判斷為不需要、輸出格式無效、發送未完整成功或安全閘門失效時都會停止。`max_follow_ups` 預設為 1，可設 0 到 3，代表初始訊息之外最多追加幾句；`delay_seconds` 預設為 2，可設 0 到 10 秒。
