...
- AI が次の 1 通をすぐ送るか判断する任意機能を追加しました。1 回の能動メッセージにつき、追加送信は 0-3 通に制限されます。
...
`immediate_follow_up_settings` はデフォルトで無効です。有効にすると、正常に送信された各能動メッセージの後で、AI がもう 1 通必要かを判断します。`max_follow_ups` のデフォルトは 1、設定範囲は追加 0-3 通です。`delay_seconds` のデフォルトは 2、設定範囲は 0-10 秒です。停止判断、無効な判定形式、不完全な送信、ユーザーの新しい発言、セッション無効化、またはおやすみ時間で直ちに終了します。
