...
`human_like_settings` はデフォルトで無効で、個人チャットとグループチャットで利用できます。有効にすると、通常返信と即時フォローアップの待機時間は既定で 1～3 秒（0.1 秒単位に対応）となり、メッセージ長と夜間に応じて自然に調整されます。`inbound_debounce_seconds` は連続入力をまとめる静穏時間で、0 にすると各メッセージを個別に処理しながら真人らしい待機時間は維持します。会話熱量は各セッションの `immediate_follow_up_settings` で設定します。

### 個人チャットの自動確認／再訪

`auto_check_settings` はデフォルトで無効で、個人チャットとグループチャットで利用できます。安定した自適応リズムで確認し、トップレベルの `context_analysis_llm_provider_id`（旧ネスト設定は fallback）に最近の会話を判定させます。モデルは `{"send_message": true|false, "message": "...", "next_check_minutes": 180}` を返せます。時間は最短／最長間隔に制限され、旧 2 フィールド形式も利用できます。`guidance` で普段会話する時間帯などを自然言語で指定できます。
