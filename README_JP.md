...
`human_like_settings` はデフォルトで無効で、個人チャットにのみ適用されます。有効にすると、メッセージ長と夜間に応じてフォローアップの待機時間を自然に調整します。ユーザーの活動で会話の熱量が上がり、能動メッセージで下がる状態を保存し、能動メッセージの Prompt に反映します。未返信のクールダウンと毎時／毎日の送信上限も設定でき、ユーザーが返信するとクールダウンが解除されます。

### 個人チャットの自動確認／再訪

`auto_check_settings` はデフォルトで無効で、個人チャットにのみ適用されます。既存のスケジュールを利用して確認時刻を決め、選択した交流スタイルまたはカスタムの最短／最長間隔で制限します。確認時には最近の会話履歴を読み込み、`context_aware_settings.llm_provider_id` に設定したプロバイダーへ `{"send_message": true|false, "message": "..."}` 形式で送信可否を判断させます。送信しない場合は次回確認だけを再設定し、未返信回数は増加しません。送信する場合は通常の能動メッセージ送信、履歴保存、即時フォローアップの流れを利用します。
