...
会話熱量は各セッションの `immediate_follow_up_settings` で設定します。`initial_heat_score` は 50、`user_activity_delta` は 15、`proactive_delivery_delta` は -5 が既定値で、値は 0～100 に収められます。即時フォローアップを有効にすると、LLM は現在の熱量ラベルを受け取ります。通常返信には別の真人風待機や入力デバウンスを追加しません。
